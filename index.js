...
/*
{
  ...
  const adminLog = prompt("Введіть пароль");
  if (!adminLog) {
    message = "Отменено пользователем!";
  } else if (adminLog === ADMIN_PASSWORD) {
...
//------------------------------------------------------------------------------------------------------------------------------------------------------\\
// ========== PR - 4
// На счету пользователя есть 23580 кредитов, значение хранится в переменной credits (создай и присвой). Пользователь решает купить ремонтных дроидов, которые
// стоят по 3000 кредитов за штуку. Цена одного дроида хранится в переменной pricePerDroid (создай и присвой).
...
let count = prompt("Скільки ви хочете купити друїдів"); //count - яке число введе користувач
if (!count) {
  result = "Отменено пользователем!";
} else if (isNaN(count[0]) || isNaN(count)) {
  result = "Введіть число";
} else {
  totalPrice = pricePerDroid * count; //общую цену заказа и сохраняет в переменной totalPrice.
}
